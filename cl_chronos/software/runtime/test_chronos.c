...
int main(int argc, char **argv) {
	...
	if (strcmp(str_app, "sssp_fs") ==0) {
		app = APP_SSSP_FS;
	}
	if (strcmp(str_app, "residual_bp") ==0) {
		app = APP_RBP;
	}
...
int test_chronos(int slot_id, int pf_id, int bar_id, FILE* fg, int app) {
	...
	for (int i=0;i<N_TILES;i++) {
		...
		// Do not dequeue a task with a timestamp larger by this much than the gvt // <TODO> (leo) play around with this for bp
		if (NO_ROLLBACK) {
			// astar - 900
			// sssp - 5000
	...
	switch (app) {
		...
		case APP_SILO:
			...
			break;
		case APP_RBP:
			printf("Verification for RBP not implemented yet\n");
			// <TODO> (leo) add verif for RBP
			break;
