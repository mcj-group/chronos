...
void visit_node_task(uint ts, uint vid) {
      ...
      float cur_dist = (float) dist[vid];
      if (cur_dist <= (float) ts) {
         return;
      }

      undo_log_write((uint *) &(dist[vid]), *((uint *) (&cur_dist)));
      dist[vid] = (float) ts;
      for (int i = edge_offset[vid]; i < edge_offset[vid+1]; i++) {
         int neighbor = edge_neighbors[i*2];
         float weight = *((float *) (&edge_neighbors[i*2+1]));
         float new_ts = ((float) ts) + weight;

         enq_task_arg0(VISIT_NODE_TASK, (uint) new_ts, neighbor);
      }
}
